...
SELECT
	@menu_id_candidate, '会员管理', 'member', NULL, '/admin/gym/member/index', @parent_menu, 'ele-User',
...
WHERE @parent_menu IS NOT NULL
	AND NOT EXISTS (
		...
		FROM sys_menu
		WHERE parent_id = @parent_menu
			AND path = '/admin/gym/member/index'
			AND del_flag = '0'
	);
...
SELECT
	@btn_add_id, '会员管理新增', NULL, 'gym_member_add', NULL, @menu_entity, NULL,
	'1', 1, '0', NULL, '1', 'admin', NOW(), 'admin', NOW(), '0'
FROM dual
WHERE @menu_entity IS NOT NULL
	AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE permission = 'gym_member_add' AND del_flag = '0');
...
SELECT
	@btn_edit_id, '会员管理修改', NULL, 'gym_member_edit', NULL, @menu_entity, NULL,
	'1', 2, '0', NULL, '1', 'admin', NOW(), 'admin', NOW(), '0'
FROM dual
WHERE @menu_entity IS NOT NULL
	AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE permission = 'gym_member_edit' AND del_flag = '0');
...
SELECT
	@btn_del_id, '会员管理删除', NULL, 'gym_member_del', NULL, @menu_entity, NULL,
	'1', 3, '0', NULL, '1', 'admin', NOW(), 'admin', NOW(), '0'
FROM dual
WHERE @menu_entity IS NOT NULL
	AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE permission = 'gym_member_del' AND del_flag = '0');
...
SELECT
	@btn_export_id, '会员管理导出', NULL, 'gym_member_export', NULL, @menu_entity, NULL,
	'1', 4, '0', NULL, '1', 'admin', NOW(), 'admin', NOW(), '0'
FROM dual
WHERE @menu_entity IS NOT NULL
	AND NOT EXISTS (SELECT 1 FROM sys_menu WHERE permission = 'gym_member_export' AND del_flag = '0');
	...
	(
		...
		WHERE role_id = @admin_role_id
			AND menu_id IN (
				@parent_menu,
				@menu_entity,
				(SELECT menu_id FROM sys_menu WHERE permission = 'gym_member_add' AND del_flag = '0' LIMIT 1),
				(SELECT menu_id FROM sys_menu WHERE permission = 'gym_member_edit' AND del_flag = '0' LIMIT 1),
				(SELECT menu_id FROM sys_menu WHERE permission = 'gym_member_del' AND del_flag = '0' LIMIT 1),
				(SELECT menu_id FROM sys_menu WHERE permission = 'gym_member_export' AND del_flag = '0' LIMIT 1)
			)
