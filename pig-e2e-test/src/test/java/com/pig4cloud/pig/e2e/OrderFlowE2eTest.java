...
public class OrderFlowE2eTest extends E2eBaseTest {
	...
	public void testCompleteOrderFlow() {
		...
		// Step 1: 创建买单
		log.info("Step 1: 创建买单");
		String buyOrderId = createOrder("BUY", "USDT", BigDecimal.valueOf(100), BigDecimal.valueOf(1.5));
		assertNotNull(buyOrderId, "买单创建失败");
		log.info("买单创建成功，订单 ID: {}", buyOrderId);

		// Step 2: 创建卖单（价格匹配，触发撮合）
		log.info("Step 2: 创建卖单（触发撮合）");
		String sellOrderId = createOrder("SELL", "USDT", BigDecimal.valueOf(50), BigDecimal.valueOf(1.5));
		assertNotNull(sellOrderId, "卖单创建失败");
		log.info("卖单创建成功，订单 ID: {}", sellOrderId);

		// Step 3: 等待撮合完成，轮询查询买单状态
		log.info("Step 3: 等待撮合完成，轮询查询买单状态");
		Response buyOrderResponse = pollUntil(
		...
		log.info("买单撮合完成，当前状态: {}", buyOrderResponse.jsonPath().getString("status"));

		// Step 4: 查询卖单状态
		log.info("Step 4: 查询卖单状态");
		Response sellOrderResponse = queryOrder(sellOrderId);
		String sellOrderStatus = sellOrderResponse.jsonPath().getString("status");
		...
		assertEquals(BigDecimal.valueOf(1.5).doubleValue(),
				((Number) firstTrade.get("price")).doubleValue(),
				"成交价格应为 1.5");
		log.info("成交详情验证通过：成交 ID={}, 成交价格={}, 成交数量={}",
				firstTrade.get("tradeId"), firstTrade.get("price"), firstTrade.get("quantity"));
	...
	public void testMarketOrderImmediateExecution() {
		...
		// Step 1: 先创建限价卖单（挂单）
		log.info("Step 1: 创建限价卖单（挂单）");
		String limitSellOrderId = createOrder("SELL", "BTC", BigDecimal.valueOf(0.1), BigDecimal.valueOf(50000));
		log.info("限价卖单创建成功，订单 ID: {}", limitSellOrderId);

		// Step 2: 确认挂单进入 MATCHING 状态
		log.info("Step 2: 确认限价卖单进入 MATCHING 状态");
		pollUntil(
				() -> queryOrder(limitSellOrderId),
				response -> "MATCHING".equals(response.jsonPath().getString("data.status")),
				"限价卖单未进入 MATCHING 状态"
		);
		log.info("限价卖单已进入 MATCHING 状态");

		// Step 3: 创建市价买单（应立即成交）
		log.info("Step 3: 创建市价买单（应立即成交）");
		String marketBuyOrderId = createMarketOrder("BUY", "BTC", BigDecimal.valueOf(0.1));
		log.info("市价买单创建成功，订单 ID: {}", marketBuyOrderId);

		// Step 4: 验证市价单立即完全成交
		log.info("Step 4: 验证市价单立即完全成交");
		Response marketOrderResponse = pollUntil(
				() -> queryOrder(marketBuyOrderId),
				response -> "FILLED".equals(response.jsonPath().getString("data.status")),
				"市价买单未完全成交"
		);
		assertEquals("FILLED", marketOrderResponse.jsonPath().getString("status"), "市价单应立即完全成交");
		log.info("市价单已完全成交");
	...
	public void testPartialOrderFill() {
		...
		String buyOrderId = createOrder("BUY", "USDC", BigDecimal.valueOf(100), BigDecimal.valueOf(3000));
		log.info("大额买单创建成功，订单 ID: {}", buyOrderId);

		// Step 2: 创建小额卖单（只能部分成交）
		log.info("Step 2: 创建小额卖单（数量 30，只能部分成交买单）");
		String sellOrderId = createOrder("SELL", "USDC", BigDecimal.valueOf(30), BigDecimal.valueOf(3000));
		...
		assertEquals("PARTIALLY_FILLED", buyOrderResponse.jsonPath().getString("status"), "买单应为部分成交");

		// 验证已成交数量
		BigDecimal filledQuantity = new BigDecimal(buyOrderResponse.jsonPath().getString("filledQuantity"));
		assertEquals(BigDecimal.valueOf(30).compareTo(filledQuantity), 0, "已成交数量应为 30");
		log.info("买单部分成交验证通过，已成交数量: {}", filledQuantity);

		// Step 4: 验证卖单完全成交
		log.info("Step 4: 验证卖单完全成交");
		Response sellOrderResponse = queryOrder(sellOrderId);
		assertEquals("FILLED", sellOrderResponse.jsonPath().getString("status"), "卖单应完全成交");
		log.info("卖单已完全成交");
	...
	private void verifyBalanceChange() {
		// 查询当前用户余额
		Response balanceResponse = authenticatedRequest()
				.when()
				.get(BALANCE_API)
				.then()
				.statusCode(200)
